...
Para los datos de entrenamiento, se obtiene un MAE de 14.509135, y un R2 de 0.485746.
...
Para los datos de validación del año 2019, se obtiene un MAE de 17.438356, y un R2 de 0.433695.
...
### Variacion entre train y val (2019)
...
Para los datos de validación del año 2020, se obtiene un MAE de 47.422131, y un R2 de -1.17278. Este MAE tan alto y este R2 negativo indican que el modelo se ajusta muy pobremente a los datos del año 2020. 
...
```{r message=FALSE, warning=FALSE, echo=F}
datos_lm1_val_2020 <- datos_val_2020 %>% group_by(FECHA, FESTIVIDAD, 
                                        DIA_SEMANA) %>% count(name = "NRO_ACCID")

datos_lm1_val_2020_pred <- datos_lm1_val_2020[,-c(4)]
predicted <- round(predict(lm1, newdata=datos_lm1_val_2020_pred, type="response"))
actual <- datos_lm1_val_2020$NRO_ACCID
lm1_val_2020_mse <- MSE(predicted, actual) # MSE
lm1_val_2020_mae <- MAE(predicted, actual) # MAE
lm1_val_2020_r2 <- R2_Score(predicted, actual) # R2
sprintf("MSE: %f, MAE: %f, R2: %f", lm1_val_2020_mse, lm1_val_2020_mae, lm1_val_2020_r2)
```

### Variacion entre train y val (2020)
...
Para los datos de entrenamiento, se obtiene un MAE de 4.747485, y un R2 de 0.933371.
...
Para los datos de validación del año 2019, se obtiene un MAE de 5.048847, y un R2 de 0.926052. Estos valores son mucho mejores que los obtenidos con el modelo anterior.
...
Para los datos de validación del año 2020, se obtiene un MAE de 10.497470, y un R2 de 0.116423. El modelo se ajusta ligeramente mejor a los datos de 2020 respecto al caso anterior, pero sigue siendo muy inadecuado para predecir la accidentalidad de este año. Tal como se explicó anteriormente, no es adecuado validar el modelo con estos datos.
...
#            data = datos_lm4_tr)
lm4 <- glm(NRO_ACCID ~ FESTIVIDAD+DIA_SEMANA+GRAVEDAD, family = "poisson",
           data = datos_lm4_tr)
```
...
predicted <- round(predict(lm4, newdata=datos_lm4_tr_pred, type="response"))
...
```{r message=FALSE, warning=FALSE, echo=F}
datos_lm4_val <- datos_val_2019 %>% group_by(FECHA, FESTIVIDAD, 
                                        DIA_SEMANA, GRAVEDAD) %>% count(name = "NRO_ACCID")

datos_lm4_val_pred <- datos_lm4_val[,-c(5)]
predicted <- round(predict(lm4, newdata=datos_lm4_val_pred, type="response"))
...
```{r message=FALSE, warning=FALSE, echo=F}
datos_lm4_val <- datos_val_2020 %>% group_by(FECHA, FESTIVIDAD, 
                                        DIA_SEMANA, GRAVEDAD) %>% count(name = "NRO_ACCID")
